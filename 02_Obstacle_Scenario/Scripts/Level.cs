...
public class Level : MonoBehaviour {
    ...
    public delegate void PipePassed();
    public event PipePassed eventPipePassed;
...
private void SpawnSkyCollider() {
        groundList = new List<Transform>();
        Transform groundTransform;
        float groundY = 50;
        float groundWidth = 192f;
        groundTransform = Instantiate(GameAssets.GetInstance().pfSkyCollider, new Vector3(0, groundY, 0), Quaternion.identity);
        groundList.Add(groundTransform);
        groundTransform = Instantiate(GameAssets.GetInstance().pfSkyCollider, new Vector3(groundWidth, groundY, 0), Quaternion.identity);
        groundList.Add(groundTransform);
        groundTransform = Instantiate(GameAssets.GetInstance().pfSkyCollider, new Vector3(groundWidth * 2f, groundY, 0), Quaternion.identity);
        groundList.Add(groundTransform);
    }
    ...
    public Pipe GetNextPipe() {
        foreach (Pipe currentPipe in pipeList) {
            if (currentPipe.GetXPosition() > BIRD_X_POSITION && currentPipe.IsBottom()) {
              return currentPipe;
            }
        }
        return null;
